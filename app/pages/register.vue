<template>
  <div class="h-screen  dark:bg-black bg-zinc-100 flex ">
    <div class="dark:bg-zinc-900 bg-zinc-50  w-[516px] p-8 px-10 flex flex-col">
        <Logo />
        <div class="flex flex-col flex-1 justify-center">
            ...
            <p class="dark:text-white text-sm">
                Already registered? 
                <NuxtLink to="/login" class="text-yellow-500 font-bold underline">Log in</NuxtLink>
                to your account
            </p>
            <UForm :schema="schema" :state="state" @submit="onSubmit" class="mt-8 flex-col flex gap-6">
                <UFormField label="Email Address" name="email" :ui="{label: 'dark:text-white'}">
                    <UInput v-model="state.email" type="email" placeholder="you@example.com" :ui="{base: 'dakr:text-white dark:bg-zinc-700'}" class="w-full"/>
                </UFormField>
                ...
                <UButton :loading="blockButton" :disabled="blockButton" type="submit" trailing-icon="i-lucide-arrow-right" class="cursor-pointer rounded-lg w-full bg-amber-400 hover:bg-amber-600 active:bg-amber-600 text-black flex justify-center mt-4" size="xl">Sign up</UButton>
            </UForm>
        </div>
...
const onSubmit = async (event: FormSubmitEvent<z.output<typeof schema>>) => {
    try {
        ...
        toast.add({ title: 'Success', description: 'The user has been registered', color: 'success' })
        return navigateTo('/')
    } catch (error: any) {
